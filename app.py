...
class CLIENT:
            ...
            try:
                if not command_queue.empty():
                    ...
                    if command_type == 'join_and_emote':
                        ...
                        for uid in uids:
                            if uid:
                                logger.info(f"[API] Performing emote {emote_id} for UID: {uid}")
                        ...
                        logger.info(f"[API] Command completed successfully")
    ...
    async def MaiiiinE(self):
        # قراءة بيانات الحساب من متغيرات البيئة
        Uid = os.environ.get('FREE_FIRE_UID', '4606641421')
        Pw = os.environ.get('FREE_FIRE_PASSWORD', 'xza-team_9B30R_RAYAN_2026_account')
        ...
        logger.info(f"API Server running on port {os.environ.get('PORT', 5000)}")
...
async def StarTinG():
    # الحصول على رقم المنفذ من متغيرات البيئة (مهم لـ Render)
    ...
    flask_thread.start()
    logger.info(f"[Flask] API server starting on http://0.0.0.0:{port}")
    
    # انتظار بدء Flask
    await asyncio.sleep(2)
    
    # حلقة التشغيل الرئيسية مع إعادة التشغيل التلقائي عند انتهاء التوكن
    while True:
        try:
            # تشغيل لمدة 7 ساعات ثم إعادة تشغيل (لأن التوكن ينتهي)
            await asyncio.wait_for(client.MaiiiinE(), timeout=7 * 60 * 60)
        except asyncio.TimeoutError:
            logger.info("Token Expired! Restarting...")
        except Exception as e:
            import traceback
            logger.error(f"Error TCP - {e} => Restarting...")
            traceback.print_exc()
        finally:
            # تنظيف الاتصالات قبل إعادة التشغيل
            if client.whisper_writer:
                try:
                    client.whisper_writer.close()
                    await client.whisper_writer.wait_closed()
                except:
                    pass
            if client.online_writer:
                try:
                    client.online_writer.close()
                    await client.online_writer.wait_closed()
                except:
                    pass
